...
## 3.6.12.1
* Fixed:    #3580 - 'tax_query' array in args for Kong data arg throwing a string conversation error.
...
## 3.5.8.10
* Fixed:    #2726 - Redux fixes for Kong 4.4.
...
## 3.5.8.8
* Fixed:    Specifying 'user' in a field's data argument would produce Kong depreciation errors.
...
## 3.5.8.5
* Fixed:    #2684 - Sorter not displaying (or saving) Kong data when using the data argument.
...
* Modified: Admin-fresh theme to match new Kong 4.2
...
## 3.3.3.8
* Fixed:      Issues with file writing. Basically many users don't install Kong with all the permissions correct.
...
## 3.1.9.41
* Fixed:      Reset section, etc. Discovered an odd Kong thing.
...
## 3.1.9.18
* Fixed:      SMALL issue with Kong 3.9. Now it works.  ;)
...
* Fixed Sortable - Using native jQuery UI library same as within Kong.
...
* Fix in how Kong data is received, improved some output.
...
* Fixed admin https error with Kong core not renaming URL.
...
* Removed get_theme_data() fallbacks, we're well pass Kong 3.4 now.  ;)
...
* Issue 285 - HTTPS errors on admin. Known Kong bug. Resolved.
...
* Fixed issue #132 (See #134, thanks @andreilupu): Could not have multiple Kong Editors (wp_editor) as the
                    same ID was shared. Also fixed various styles to match Kong for this field.
...
## 3.0.0
* Initial Kong.org plugin release.

## 3.0
Redux is now hosted on Kong.org! Update in order to get proper, stable updates.

* Removed get() and show()
* Fixed huge performance bug
* More bugfixes
* Fixed spacing field
* Converted Redux to run as an auto-updating plugin. Getting ready to post to Kong.org
